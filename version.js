...
//         console.log("Agent OUTBOUND STATISTICS:", outboundStatistics);
//         console.log("AGENT INBOUBD Statistics:", agentStatistics);
//         Object.keys(agentStatistics).forEach(agentId => {
//             console.log("processing GENT", agentId)
...
//             const outboundStats = outboundStatistics[outboundAgentId] || { totalOutboundCalls: 0 };
//             let row = document.querySelector(`#agentTable tbody tr[data-agent-number="${String(outboundAgentId)}"]`);
//             if (row) {
//                 row.querySelector('.outbound-calls').innerText = outboundStats.totalOutboundCalls;
//             } else {
//                 console.warn(`No row found for outbound agent number: ${outboundAgentId}`);
//             }
//         });
...
// Global variable to store the agent list
let agentList = {};

// Retrieve agent details and assign to agentList
function getConfigAgents() {
    // Subscribe to the AgentList topic to receive agent data
    Cti.getList("agent");

    // Listen for responses on the AgentList topic
    Cti.Topic("AgentList").subscribe((response) => {
        if (response) {
            // Log the received agent data for debugging
            console.log("Received agent data:", response);

            // Store the agent list globally for further use
            agentList = response;

            // Log the number of agents received
            const agentCount = response.length;
            console.log(`Total agents received: ${agentCount}`);

            // Call the function to update the statistics table
        ...
        if (outboundStats) {
            const row = `
                <tr data-agent-number="${agent.id}">
                    ...
                    <td class="total-outbound-talk-time">${(outboundStats.totalOutboundTalkTime)}</td>
                    <td class="last-outbound-call">${outboundStats.lastOutboundCallTime ? (outboundStats.lastOutboundCallTime) : "N/A"}</td>
                    <td class="avg-outbound-handle-time">${(outboundStats.averageOutboundHandleTime)}</td>
...
function calculateAnswerRate(stats) {
    const { answeredCalls, totalCalls } = stats;
    return totalCalls > 0 ? `${((answeredCalls / totalCalls) * 100).toFixed(2)}%` : "0%";
}
            ...
            if (data.msgType === "QueueList") {
                ...
                fetchData('queue_logs').then(queueLogs => {
                    ...
                    queueLogs.forEach(log => {
                        ...
                        console.log("--------------------------------------------------", todayAt7AM, log.time);
